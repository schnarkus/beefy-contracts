...
contract StrategyBalancerMultiReward is StratFeeManagerInitializable {
    ...
    function swapRewardsToNative() internal {
        ...
        for (uint i; i < rewardTokens.length; ++i) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                ...
                } else if (rewards[rewardTokens[i]].routerType == BeefyBalancerStructs.RouterType.TRADER_JOE) {
                    UniV3Actions.swapLiquidityBook(
                        rewards[rewardTokens[i]].router,
                        rewards[rewardTokens[i]].joePath,
                        bal,
                        address(this)
                    );
                }
    ...
    function addRewardToken(
        ...
        ITraderJoeRouter.Path memory _joePath,
        uint _minAmount
    ) external onlyOwner {
        ...
        if (_token == output) {
            ...
            rewards[_token].joePath = _joePath;
