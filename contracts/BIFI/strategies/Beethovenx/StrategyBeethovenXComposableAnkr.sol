...
import "../Balancer/BeefyBalancerStructs.sol";

interface IBalancerPool {
    function getPoolId() external view returns (bytes32);
}

contract StrategyBeethovenXComposableAnkr is StratFeeManagerInitializable {
    ...
    bytes32 public ankrFTMSwapPoolId;
    address public rewarder;
    address public solidlyRouter;

    // Routes
    ISolidlyRouter.Routes[] public secondOutputToankrFTMRoute;
    ...
    function initialize(
        uint256 _chefPoolId,
        bytes32[] calldata _outputSwapPoolId,
        bytes32[] calldata _ankrFTMSwapPoolId,
        ISolidlyRouter.Routes[] calldata _secondOutputToankrFTMRoute,
        BeefyBalancerStructs.BatchSwapStruct[] calldata _nativeToWantRoute,
        address[] calldata _nativeToWantAssets,
        CommonAddresses calldata _commonAddresses
    ) public initializer {
        __StratFeeManager_init(_commonAddresses);

        for (uint i; i < _secondOutputToankrFTMRoute.length; ) {
            secondOutputToankrFTMRoute.push(_secondOutputToankrFTMRoute[i]);
            unchecked {
                ++i;
            }
        }

        for (uint j; j < _nativeToWantRoute.length; ) {
            nativeToWantRoute.push(_nativeToWantRoute[j]);
            unchecked {
                ++j;
            }
        }

        chef = address(0x8166994d9ebBe5829EC86Bd81258149B87faCfd3);
        output = address(0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e);
        native = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        solidlyRouter = address(0x2aa07920E4ecb4ea8C801D9DFEce63875623B285);

        chefPoolId = _chefPoolId;
        rewarder = IBeethovenxChef(chef).rewarder(chefPoolId);
        outputSwapPoolId = _outputSwapPoolId[0];
        ankrFTMSwapPoolId = _ankrFTMSwapPoolId[0];
        secondOutput = secondOutputToankrFTMRoute[0].from;
        ankrFTM = secondOutputToankrFTMRoute[secondOutputToankrFTMRoute.length - 1].to;
        nativeToWantAssets = _nativeToWantAssets;
    ...
    function swapRewardsToNative() internal {
        ...
        if (outputBal > 1 ether) {
            balancerSwap(outputSwapPoolId, output, native, outputBal);
        }

        // second output to ankrFTM
        uint256 rewardBal = IERC20(secondOutput).balanceOf(address(this));
        if (rewardBal > 0) {
            ISolidlyRouter(solidlyRouter).swapExactTokensForTokens(
                rewardBal,
                0,
                secondOutputToankrFTMRoute,
                address(this),
                block.timestamp
            );
        }
        ...
        // ankrFTM to native
        uint256 ankrFTMBal = IERC20(ankrFTM).balanceOf(address(this));
        if (ankrFTMBal > 0) {
            balancerSwap(ankrFTMSwapPoolId, ankrFTM, native, ankrFTMBal);
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            (nativeOut, ) = ISolidlyRouter(solidlyRouter).getAmountOut(outputBal, secondOutput, native);
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ankrFTM).safeApprove(unirouter, type(uint).max);
        ...
        IERC20(secondOutput).safeApprove(solidlyRouter, 0);
        IERC20(secondOutput).safeApprove(solidlyRouter, type(uint).max);
        if (secondOutput != native) {
            IERC20(native).safeApprove(solidlyRouter, type(uint).max);
        }
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(ankrFTM).safeApprove(unirouter, 0);
        ...
        IERC20(secondOutput).safeApprove(solidlyRouter, 0);
        IERC20(native).safeApprove(solidlyRouter, 0);
    }
    ...
    function secondOutputToankrFTM() external view returns (address[] memory) {
        ISolidlyRouter.Routes[] memory _route = secondOutputToankrFTMRoute;
